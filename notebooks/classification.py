...
def scorers():
    kwargs = {'average':'weighted'}#{'average':'samples'}
    scoring = {'accuracy' : make_scorer(accuracy_score), 
               'precision' : make_scorer(precision_score, **kwargs),
               'recall' : make_scorer(recall_score,**kwargs), 
               'f1_score' : make_scorer(f1_score, **kwargs)
              }
    return scoring

def train_validate_test_model(X_train, y_train, X_test, y_test, model, params, min_obs, num_features, oversample, task, scaler, precision_digits = 4):
    
    # Label Binarizer
    if 'binary' in task.__name__.lower():
        lb = LabelBinarizer()
        y_train = np.array([number[0] for number in lb.fit_transform(y_train)])

    # Train & cross-validate
    grid_search = GridSearchCV(model, params, cv=StratifiedKFold(2),
                               scoring=scorers(), refit='f1_score',
